...
export function getSectionListData(data) {
  ...
  // Initialize an object to hold sections
  const sections = {};

  // Iterate over the data and organize items into sections based on their category
  data.forEach((item) => {
    // Check if the category already exists in sections
    if (!sections[item.category]) {
      // If not, create a new section with an empty array
      sections[item.category] = [];
    }
    // Push the item to the corresponding section
    sections[item.category].push(item);
  });

  // Convert the sections object into an array of section objects
